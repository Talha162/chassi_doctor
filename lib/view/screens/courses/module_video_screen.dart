...
class _ModuleVideosScreenState extends State<ModuleVideosScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      final videos = await _service.getVideosForModule(widget.module.id);

      Map<String, VideoProgress> progressMap = {};
      if (userId != null) {
        final allVideoProgress = await _service.getVideoProgressForUser(userId);
      ...
      body: _isLoading
          ...
          : ListView(
              ...
              children: [
                ...
                if (_videos.isEmpty)
                  MyText(text: 'No videos added yet.', size: 14)
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const BouncingScrollPhysics(),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final watchedSeconds = progress?.watchedSeconds ?? 0;
                      final totalSeconds = video.durationSeconds ?? 1;
                      final percent = (watchedSeconds / totalSeconds).clamp(
                        0.0,
                        1.0,
                      );
                      ...
                      return Padding(
                        padding: EdgeInsets.only(
                          bottom: index < _videos.length - 1 ? 10 : 0,
                        ),
                        child: InkWell(
                          onTap: () {
                            final userId =
